...
import { changeTransaction, setTransactionPaid, payDebt } from './utils/transactionActions';
...
export function App() {
  ...
  const [activeTab, setActiveTab] = useState<'chat' | 'overview' | 'transactions' | 'debts' | 'projection' | 'banks'>('chat');
  ...
  const handleSaveTransaction = (
    ...
    setData((prev) => {
      if (id) {
        ...
        return old ? changeTransaction(prev,id,{...old,...txData}) : prev;
      } else {
        // Criação
  ...
  const handleDeleteTransaction = (id:string) => {
    setData(prev=>changeTransaction(prev,id));
    showToast('Lançamento excluído e totais recalculados.');
  };
  const handleToggleTransactionStatus = (id:string) => {
    setData(prev=>{const tx=prev.transactions.find(t=>t.id===id);return tx?setTransactionPaid(prev,id,tx.status!=='completed',new Date().toISOString().slice(0,10)):prev;});
    showToast('Situação atualizada.');
  };
  const handlePayExpense = (id:string) => {
    setData(prev=>setTransactionPaid(prev,id,true,new Date().toISOString().slice(0,10)));
    showToast('Pagamento conferido.');
  };
   ...
   */
  const handlePayDebtInstallment = (id:string) => {
    setData(prev=>payDebt(prev,id,selectedMonthYear,new Date().toISOString().slice(0,10)));
    showToast('Parcela conferida. Um pagamento já registrado não é repetido.');
  };
    ...
    <div className="min-h-screen bg-stone-50 text-stone-900 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {activeTab === 'projection' && (
          ...
          <RealMoneySliceBar summary={financialSummary} selectedMonthName={selectedMonthName} selectedMonthYear={selectedMonthYear}/>
          <MonthlyProjection
            summary={financialSummary}
            transactions={data.transactions}
            debts={data.debts}
